...
/**
 * 拦截响应并记录错误日志，并注入业务状态码
 */
function apiResponseInterceptor(req, res, next) {
    const oldJson = res.json;
    
    res.json = function(body) {
        // 确保响应体是一个对象且属于 API 接口
        if (body && typeof body === 'object' && (req.originalUrl.startsWith('/api') || req.path.startsWith('/api'))) {
            // 如果业务代码未定义，则根据 HTTP 状态码进行映射
            if (body.code === undefined) {
                if (res.statusCode === 200 || res.statusCode === 201) {
                    body.code = StatusCode.SUCCESS;
                } else if (res.statusCode === 401) {
                    body.code = StatusCode.UNAUTHORIZED;
                } else if (res.statusCode === 403) {
                    body.code = StatusCode.FORBIDDEN;
                } else if (res.statusCode === 404) {
                    body.code = StatusCode.NOT_FOUND;
                } else if (res.statusCode === 422 || res.statusCode === 400) {
                    body.code = StatusCode.BAD_REQUEST;
                } else {
                    body.code = StatusCode.INTERNAL_ERROR;
                }
            }
        }
