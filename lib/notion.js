...
class NotionClient {
    ...
    async request(method, path, data = null, params = null) {
        ...
        try {
            ...
            await db.logApiCall(url, method, { data, params }, response.status, response.data, true, null);
            return response.data;
        } catch (error) {
            const status = error.response ? error.response.status : 500;
            const errorData = error.response ? error.response.data : error.message;
            await db.logApiCall(url, method, { data, params }, status, errorData, false, error.message);
            throw error;
        }
    }
    ...
    async getDataSourceId(databaseId) {
        const data = await this.request('GET', `/databases/${databaseId}`);
        if (data.data_sources && data.data_sources.length > 0) {
            return data.data_sources[0].id;
        }
        throw new Error('No data_source_id found in database response');
    }
